...
async function redirectToBoard() {
    const check = await uploadTaskAddTaskSection()
    if (!check) {
        return
    }
    await switchContent()
...
async function switchContentAddTaskShowSuccess() {
    const container = document.querySelector('.add-task-section-body')
...
async function switchContentAddTaskShowSuccessReset() {
    const container = document.querySelector('.add-task-section-body')
    const slideOutAnimation = container.animate(transformArrayStart, animationAttributeObjectStart, overflowTimer());
    await slideOutAnimation.finished;
    container.innerHTML = switchContentShowSuccessResetTemplate()
    container.animate(transformArrayFinish, animationAttributeObjectFinish);
    return;
}

function switchContentShowSuccessResetTemplate() {
    let template;
    template = `
                    <div class="floating-add-task-body-first-half">
                        <div class="float-add-task-title">
                            ...
                            <div class="flt-add-tsk-title-input-box">
                                <input type="text" placeholder="Enter a title" id="flt-title-add" class="flt-title-add">
                            </div>
                        </div>
                    ...
                    <div class="floating-add-task-body-second-half">
                        <div class="float-add-task-priority add-task-section-priority">
                        ...
                        <div class="float-add-task-category">
                            ...
                            <div class="flt-add-tsk-category-input-box" onclick="categoryInputActive()" id="category">
...
async function switchContent() {
    await switchContentAddTaskShowSuccess()
    await setTimer(1000)
    await switchContentAddTaskShowSuccessReset()
    await setTimer(1000)
    return
}
